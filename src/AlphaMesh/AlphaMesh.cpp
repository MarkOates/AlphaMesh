...
std::vector<ALLEGRO_VERTEX> AlphaMesh::build_mesh__collapse_rows_columns()
{
   AllegroFlare::TileMaps::TileMap<bool> tile_mask = build_tile_mask();
...
std::vector<::AlphaMesh::Rectangle> AlphaMesh::find_rectangles()
{
   std::vector<::AlphaMesh::Rectangle> result;
   //return result;

   //std::vector<Rectangle> find_rectangles(std::vector<std::vector<int>> &grid)
   AllegroFlare::TileMaps::TileMap<bool> grid = build_tile_mask();
   //{
   //int rows = grid.size();
   int rows = grid.get_num_rows();
   //int cols = grid[0].size();
   int cols = grid.get_num_columns();
   ...
   for (int i = 0; i < rows; ++i)
   {
      ...
      {
         //if (grid[i][j] == true)  // Found a full tile
         if (grid.get_tile(i, j) == true)  // Found a full tile
         {
            ...
            while (x2 < rows && grid.get_tile(x2, j) == true)
            //while (x2 < rows && grid[x2][j] == true)
            {
               ++x2;
            }
