...
Main::Main()
   : AllegroFlare::GameConfigurations::Complete(AlphaMesh::Game::Configurations::Main::TYPE)
...
AllegroFlare::Screens::Gameplay* Main::create_primary_gameplay_screen(AllegroFlare::Runners::Complete* runner)
{
   if (!(runner))
   {
      ...
      error_message << "[AlphaMesh::Game::Configurations::Main::create_primary_gameplay_screen]: error: guard \"runner\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AlphaMesh::Game::Configurations::Main::create_primary_gameplay_screen]: error: guard \"runner\" not met");
   }
   ...
   AlphaMesh::Gameplay::Screen *result = new AlphaMesh::Gameplay::Screen;
...
AllegroFlare::Screens::Subscreen::Screen* Main::create_primary_gameplay_subscreen(AllegroFlare::Runners::Complete* runner)
{
   ...
   {
      std::stringstream error_message;
      error_message << "[AlphaMesh::Game::Configurations::Main::create_primary_gameplay_subscreen]: error: guard \"(!primary_gameplay_subscreen)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AlphaMesh::Game::Configurations::Main::create_primary_gameplay_subscreen]: error: guard \"(!primary_gameplay_subscreen)\" not met");
   }
...
AllegroFlare::Screens::PauseScreen* Main::create_pause_screen(AllegroFlare::Runners::Complete* runner)
{
   ...
   {
      std::stringstream error_message;
      error_message << "[AlphaMesh::Game::Configurations::Main::create_pause_screen]: error: guard \"(!pause_screen)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AlphaMesh::Game::Configurations::Main::create_pause_screen]: error: guard \"(!pause_screen)\" not met");
   }
   AllegroFlare::FontBin* font_bin = runner->get_font_bin();
   AllegroFlare::BitmapBin* bitmap_bin = runner->get_bitmap_bin();
   AllegroFlare::EventEmitter* event_emitter = runner->get_event_emitter();
...
void Main::handle_primary_gameplay_screen_paused()
{
   ...
   {
      std::stringstream error_message;
      error_message << "[AlphaMesh::Game::Configurations::Main::handle_primary_gameplay_screen_paused]: error: guard \"primary_gameplay_screen\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AlphaMesh::Game::Configurations::Main::handle_primary_gameplay_screen_paused]: error: guard \"primary_gameplay_screen\" not met");
   }
...
void Main::handle_primary_gameplay_screen_unpaused()
{
   ...
   {
      std::stringstream error_message;
      error_message << "[AlphaMesh::Game::Configurations::Main::handle_primary_gameplay_screen_unpaused]: error: guard \"primary_gameplay_screen\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AlphaMesh::Game::Configurations::Main::handle_primary_gameplay_screen_unpaused]: error: guard \"primary_gameplay_screen\" not met");
   }
   // TODO: Add tests for this behavior
   // TODO: Consider having and using event_emitter from self
   AllegroFlare::EventEmitter* event_emitter = primary_gameplay_screen->get_event_emitter();
...
void Main::load_audio_controller(AllegroFlare::AudioController* audio_controller)
{
   ...
   {
      std::stringstream error_message;
      error_message << "[AlphaMesh::Game::Configurations::Main::load_audio_controller]: error: guard \"audio_controller\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AlphaMesh::Game::Configurations::Main::load_audio_controller]: error: guard \"audio_controller\" not met");
   }
...
void Main::load_last_played_session_or_start_new(AllegroFlare::GameSession* game_session)
{
   ...
   {
      std::stringstream error_message;
      error_message << "[AlphaMesh::Game::Configurations::Main::load_last_played_session_or_start_new]: error: guard \"game_session\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AlphaMesh::Game::Configurations::Main::load_last_played_session_or_start_new]: error: guard \"game_session\" not met");
   }
   // TODO: This method
...
void Main::setup_new_game_progress_and_state_info(AllegroFlare::GameSession* game_session)
{
   ...
   {
      std::stringstream error_message;
      error_message << "[AlphaMesh::Game::Configurations::Main::setup_new_game_progress_and_state_info]: error: guard \"game_session\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[AlphaMesh::Game::Configurations::Main::setup_new_game_progress_and_state_info]: error: guard \"game_session\" not met");
   }
   // TODO: This method
   //AllegroFlare::Logger::throw_error(
...
AllegroFlare::Levels::Base* Main::load_level_by_identifier(std::string identifier)
{
   ...
   AlphaMesh::Gameplay::Level *result = new AlphaMesh::Gameplay::Level();

   std::map<std::string, std::function<void()>> items_map = {
      { "", [](){
         AllegroFlare::Logger::warn_from(
            "AlphaMesh::GameConfigurations::Main::load_level_by_identifier",
   ...
   if (items_map.count(identifier) == 0)
   {
      ...
      if (!item_handled)
      {
         // item not found
         std::stringstream error_message;
         error_message << "[AlphaMesh::Game::Configurations::Main::load_level]: error: Cannot load the item with the identifier \""
