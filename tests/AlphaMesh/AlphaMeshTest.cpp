...
{};
class AlphaMesh_AlphaMeshTestWithInteractionFixture
...
TEST_F(AlphaMesh_AlphaMeshTestWithAllegroRenderingFixture,
   CAPTURE__build_mesh__collapse_rows_columns___will_produce_an_optimized_mesh)
{
   int num_rows = 32;
   int num_columns = 16;
   ALLEGRO_BITMAP *bitmap = get_bitmap_bin_ref()["investigator-01.png"];
   ...
   // Draw the mesh (optimized by rows)
   {
      ...
      auto mesh = alpha_mesh.build_mesh__collapse_rows_columns();
      al_draw_prim(&mesh[0], nullptr, bitmap, 0, mesh.size(), ALLEGRO_PRIM_TRIANGLE_LIST);
   ...
   CAPTURE__INTERACTIVE__build_mesh__collapse_rows_columns___will_produce_expected_triangles)
{
   ...
   ALLEGRO_TRANSFORM camera_transform, hud_transform;
   al_identity_transform(&camera_transform);
   ...
   while(interactive_test_wait_for_event())
   {
      ...
      switch(current_event.type)
      {
         ...
         case ALLEGRO_EVENT_TIMER: {
            ...
            ALLEGRO_FONT *font = get_any_font();
            int lh = 26;
            al_draw_textf(font, ALLEGRO_COLOR{1, 1, 1, 1}, 18, 12+lh*0, 0, "vertices: %d", mesh.size());
            al_draw_textf(font, ALLEGRO_COLOR{1, 1, 1, 1}, 18, 12+lh*1, 0, "rows: %d", alpha_mesh.get_num_rows());
   ...
   FOCUS__CAPTURE__audit_on_specific_bitmaps)
{
   ...
   ALLEGRO_BITMAP *bitmap = get_bitmap_bin_ref()["investigator-01.png"];
   //ALLEGRO_BITMAP *bitmap = get_bitmap_bin_ref()["middle.png"];
   AlphaMesh::AlphaMesh alpha_mesh(bitmap, num_rows, num_columns);
   ...
   // Draw the mesh (optimized by rows)
   {
      ...
      al_draw_textf(font, ALLEGRO_COLOR{1, 1, 1, 1}, 18, 12, 0, "vertices: %d", mesh.size());
      al_draw_textf(font, ALLEGRO_COLOR{1, 1, 1, 1}, 18, 36, 0, "rows: %d", alpha_mesh.get_num_rows());
      al_draw_textf(font, ALLEGRO_COLOR{1, 1, 1, 1}, 18, 60, 0, "columns: %d", alpha_mesh.get_num_columns());
